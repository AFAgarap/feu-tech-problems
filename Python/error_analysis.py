...
def get_input():
    ...
    try:
        ...
        operation = int(
            input(
                "Which arithmetic operation to do:\n[1] Addition (+)\n[2] Subtraction (-)\n[3] Multiplication (*)\n[4] Division (/)\nEnter choice >> "
            )
        )
    except:
...
def main():
    ...
    # for chop() value
    approximate_value = get_approximate_value(
        float("{0:.4f}".format(x)), float("{0:.4f}".format(y)), opt
    )
    ...
    # for round() value
    round_approximate_value = get_approximate_value(
        round(float("{0:.4f}".format(x)), 4), round(float("{0:.4f}".format(y)), 4), opt
    )
