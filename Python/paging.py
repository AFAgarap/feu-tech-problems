...
def main():
    ...
    virtual_page_table = set_virtual_page_table(
        virtual_page_table=virtual_page_table, size=size, word_size=word_size
    )
    ...
    physical_frame_table = get_physical_frame_table(
        page_mapping,
        physical_frame_table,
        virtual_page_table,
        sorted_mapping[-1],
    ...
    for frame_number in range(0, upperbound + 1):
        physical_frame_table.append(
            [
                ...
                virtual_page_table[page_mapping.index(frame_number)]
                if frame_number in page_mapping
                else None,
            ]
        )
