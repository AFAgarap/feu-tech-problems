    ...
    if sys.platform == "linux":
        os.system(
            'wget --output-document={}.{} "{}'.format(
                filename,
                "jpg" if choice == 1 else "mp4" if choice == 2 else "",
                media_url,
            )
        )
        print(
            ...
            if os.path.isfile(
                "{}.{}".format(
                    filename, "jpg" if choice == 1 else "mp4" if choice == 2 else ""
                )
            )
            else "Failed"
        )
...
if __name__ == "__main__":
    ...
    fetch_media(
        url=input("Enter URL: "), filename=input("Enter filename: "), choice=choice
    )
