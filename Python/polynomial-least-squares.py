#!/usr/bin/python3
# Approximation Theory : Polynomial Least Squares
...
def main():
    ...
    xs, xy = get_system_of_equations(
        x, y, n
    )  # \sum_{k=0}^{n}a_{k} \sum_{i=1}^{m}x_{i}^{j+k} = \sum_{i=1}^{m}y_{i}x_{i}^{j}, for j = 0,1,...,n
    xs = np.reshape(
        xs, ((n + 1), (n + 1))
    )  # reshape the matrix xs to solve the system of equations
...
def get_system_of_equations(x, y, n):
    xs = np.array([])
    xy = np.array([])  # xs is summation of x-values, xy is product of x- and y-values
